...
def get_colors(image, no_color):
    ...
    clf = KMeans(n_clusters = no_color, random_state=0)
    labels = clf.fit_predict(modified_image)
    counts = Counter(labels)
    counts = dict(sorted(counts.items()))
    center_colors = clf.cluster_centers_

    ordered_colors = [rgb2hex(center_colors[i]) for i in counts.keys() if not all(center_colors[i] > 250)] #Remove White colors
    
    return ordered_colors, counts
...
for (i, rect) in enumerate(rects):
    ...
    upperlip_color, upperlip_counts = get_colors(upperlip_img, 10)
    lowerlip_color, lowerlip_counts = get_colors(lowerlip_img, 10)
    ...
    lip_analysis = normalize(dict(Counter(upperlip_analysis) + Counter(lowerlip_analysis)))

    top_colours = dict(Counter(lip_analysis).most_common(10))
    lip_analysis_val = []
    for key, value in top_colours.items():
        for col_val in hex2rgb(key):
            lip_analysis_val.append(col_val)
        lip_analysis_val.append(value)
